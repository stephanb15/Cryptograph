...
while True:
    ...
    for notified_socket in read_sockets:
        if notified_socket == server_socket:
            ...
            print(f"Verbindung hergestellt {client_address[0]}:{client_address[1]} username:{user['data'].decode('utf-8')}")
            
        else:
            message = receive_message(notified_socket)
            
            if message is False:
                print(f"Verbindung getrennt {clients[notified_socket]['data'].decode('utf-8')}")
                sockets_list.remove(notified_socket)
                del clients[notified_socket]
                continue
            
            user = clients[notified_socket]
            print(f"Nachricht von {user['data'].decode('utf-8')}: {message['data'].decode('utf-8')}")
